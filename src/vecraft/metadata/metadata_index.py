...
class MetadataIndex:
    """
    A metadata vector_index supporting equality and range queries.

    - Equality queries via inverted vector_index: field -> value -> set(record_id).
    ...
    def delete(self, item: MetadataItem) -> None:
        """
        Remove a record's metadata from the vector_index.
        """
        rid = item.record_id
        for field, value in item.metadata.items():
            if isinstance(value, (list, set, tuple)):
    ...
    def get_matching_ids(self, where: Dict[str, Any]) -> Optional[Set[str]]:
        ...
        Returns None if unable to use vector_index effectively.
        """
        result_ids = None
    ...
    def serialize(self) -> bytes:
        """
        Serialize the metadata vector_index to bytes for snapshotting.
        """
        state = {
    ...
    def deserialize(self, data: bytes) -> None:
        """
        Restore the metadata vector_index from serialized bytes.
        """
