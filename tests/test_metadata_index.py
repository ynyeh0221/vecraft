...
from src.vecraft.index.metadata_index import MetadataItem, MetadataIndex


class TestMetadataIndex(unittest.TestCase):

    def setUp(self):
        """Set up a fresh index before each test."""
    ...
    def test_add_and_equality_query(self):
        ...
        result = self.index.get_matching_ids({"type": "article", "tags": "index"})
        self.assertEqual({"doc1", "doc3"}, result)
    ...
    def test_update(self):
        """Test updating items in the index."""
        self.index.add(self.item1)
        self.index.add(self.item2)
        ...
        result = self.index.get_matching_ids({"tags": "index"})
        self.assertEqual(set(), result)
    ...
    def test_delete(self):
        ...
        # Verify it's removed from equality index
        result = self.index.get_matching_ids({"type": "book"})
        self.assertEqual(set(), result)

        # Verify it's removed from range index
    ...
    def test_edge_cases(self):
        ...
        # Non-comparable values for range index
